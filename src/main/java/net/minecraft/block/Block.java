...
public class Block
{
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vector3D start, Vector3D end)
    {
        ...
        else
        {
            ...
            return new MovingObjectPosition(vec36D.addVector(pos.getX(), pos.getY(), pos.getZ()), enumfacing, pos);
        }
    }

    private boolean isVecInsideYZBounds(Vector3D point)
    {
        return point != null && point.y() >= this.minY && point.y() <= this.maxY && point.z() >= this.minZ && point.z() <= this.maxZ;
    }

    private boolean isVecInsideXZBounds(Vector3D point)
    {
        return point != null && point.x() >= this.minX && point.x() <= this.maxX && point.z() >= this.minZ && point.z() <= this.maxZ;
    }

    private boolean isVecInsideXYBounds(Vector3D point)
    {
        return point != null && point.x() >= this.minX && point.x() <= this.maxX && point.y() >= this.minY && point.y() <= this.maxY;
    }
