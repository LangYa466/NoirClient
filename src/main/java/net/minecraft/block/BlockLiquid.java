        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (j < 0)
            {
                if (!worldIn.getBlockState(blockpos).getBlock().getMaterial().blocksMovement())
                {
                    ...
                    {
                        int k = j - (i - 8);
                        vector3D = vector3D.addVector((blockpos.getX() - pos.getX()) * k, (blockpos.getY() - pos.getY()) * k, (blockpos.getZ() - pos.getZ()) * k);
                    }
                }
            }
            else if (j >= 0)
            {
                int l = j - i;
                vector3D = vector3D.addVector((blockpos.getX() - pos.getX()) * l, (blockpos.getY() - pos.getY()) * l, (blockpos.getZ() - pos.getZ()) * l);
            }
        }
