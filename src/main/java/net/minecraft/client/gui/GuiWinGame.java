    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int i1 = 0; i1 < this.field_146582_i.size(); ++i1)
        {
            ...
            if ((float)l + f + 12.0F + 8.0F > 0.0F && (float)l + f < (float)this.height)
            {
                ...
                if (s.startsWith("[C]"))
                {
                    this.fontRendererObject.drawStringWithShadow(s.substring(3), (float)(j + (i - this.fontRendererObject.getStringWidth(s.substring(3))) / 2), (float)l, 16777215);
                }
                else
                {
                    this.fontRendererObject.fontRandom.setSeed((long)i1 * 4238972211L + (long)(this.field_146581_h / 4));
                    this.fontRendererObject.drawStringWithShadow(s, (float)j, (float)l, 16777215);
                }
            }
