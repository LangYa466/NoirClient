    ...
    private void drawActivePotionEffects()
    {
        ...
        if (!collection.isEmpty())
        {
            ...
            for (PotionEffect potioneffect : this.mc.thePlayer.getActivePotionEffects())
            {
                ...
                String s1 = LocalizationHelper.translate(potion.getName());

                if (potioneffect.getAmplifier() == 1)
                {
                    s1 = s1 + " " + LocalizationHelper.translate("enchantment.level.2");
                }
                else if (potioneffect.getAmplifier() == 2)
                {
                    s1 = s1 + " " + LocalizationHelper.translate("enchantment.level.3");
                }
                else if (potioneffect.getAmplifier() == 3)
                {
                    s1 = s1 + " " + LocalizationHelper.translate("enchantment.level.4");
                }
