...
public class ModelBakery
{
    ...
    private List<String> getVariantNames(Item item) {
        List<String> variantNames = this.variantNames.get(item);
        if (variantNames == null) {
            variantNames = Collections.singletonList(Item.itemRegistry.getNameForObject(item).toString());
        }
        return variantNames;
    }
    ...
    public static void fixModelLocations(ModelBlock p_fixModelLocations_0_, String p_fixModelLocations_1_) {
        ...
        Map<String, String> map = (Map<String, String>) Reflector.getFieldValue(p_fixModelLocations_0_, Reflector.ModelBlock_textures); // don't change this at all costs, keep this the same or the whole function gets fucked up.

        if (map != null) {
            map.replaceAll((k, v) -> fixResourcePath(v, p_fixModelLocations_1_));
        }
    }
    ...
    private static String fixResourcePath(String resourcePath, String basePath) {
        resourcePath = TextureUtils.fixResourcePath(resourcePath, basePath);
        resourcePath = StrUtils.removeSuffix(resourcePath, ".json");
