...
public abstract class Entity implements ICommandSender {
    ...
    protected final Vector3D getVectorForRotation(float pitch, float yaw) {
        return new Vector3D(
                -Math.sin(-yaw * 0.017453292F - (float) Math.PI) * Math.cos(-pitch * 0.017453292F),
                Math.sin(-pitch * 0.017453292F),
                -Math.cos(-yaw * 0.017453292F - (float) Math.PI) * Math.cos(-pitch * 0.017453292F)
        );
    }

    public Vector3D getPositionEyes(float partialTicks) {
        double lerpPosX = XMathHelper.INSTANCE.lerp(partialTicks, this.prevPosX, this.posX);
        double lerpPosY = XMathHelper.INSTANCE.lerp(partialTicks, this.prevPosY, this.posY) + this.getEyeHeight();
        double lerpPosZ = XMathHelper.INSTANCE.lerp(partialTicks, this.prevPosZ, this.posZ);
        return new Vector3D(lerpPosX, lerpPosY, lerpPosZ);
    }
