    ...
    public void onLivingUpdate()
    {
        if (this.worldObj.isRemote)
        {
            ...
            if (this.func_175472_n() && this.isInWater())
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width - vector3D.x() * 1.5D, this.posY + this.rand.nextDouble() * (double)this.height - vector3D.y() * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width - vector3D.z() * 1.5D, 0.0D, 0.0D, 0.0D);
                }
            }
